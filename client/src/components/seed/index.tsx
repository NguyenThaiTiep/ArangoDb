...
export const Seed = (props: Props) => {
  ...
  const seedData = () => {
    if (amount == 0) return handelToast(0, 0, "Nhập số lượng");
  ...
  const seedCategory = () => {
    CategoryAPI_MYSQL.seedData(amount).then((res) => {
      if (res.data.status != 200) handelToast(res.data.status);
      else {
      ...
      <div className={"input-amount"}>
        <InputGroup className="mb-3">
          ...
          <FormControl
            ...
            onChange={(e) => {
              setAmount(Math.abs(e.target.value as any));
            }}
          />
          <InputGroup.Append>
            <Button
